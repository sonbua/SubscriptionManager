...
namespace SubscriptionManager.Services
{
    public class SwaggerConfig
    {
        public static void Register()
        {
            GlobalConfiguration.Configuration
                .EnableSwagger(
                    c =>
                    {
                        IncludeXmlCommentsFromBinFolder(c);
        ...
        {
            private readonly IEnumerable<ITypeDescriptionProvider> _providers;

            public ControllerlessActionOperationFilter(params string[] xmlCommentsPaths)
            {
                _providers = xmlCommentsPaths.Select(p => new XmlDocumentationTypeDescriptionProvider(p)).ToArray();
            }
            ...
            public void Apply(Operation operation, SchemaRegistry schemaRegistry, ApiDescription apiDescription)
            {
                ...
                if (descriptor != null)
                {
                    operation.summary = GetSummaries(descriptor.MessageType).FirstOrDefault() ?? operation.summary;
                }
            }
